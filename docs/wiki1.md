...
The aim of SQANTI-SIM is to simulate novel and degradated transcripts in a controlled way using as ground-truth real transcripts in the reference annotation that fit the description of the different SQANTI3 structural categories. SQANTI-SIM simulates reads from **real annotated transcripts** and modifies the GTF reference annotation pretending that the original transcripts are not annotated (novel). This allows to have a robust ground-truth and also to use **orthogonal data** (Short-read RNA-seq data, CAGE Peak data, polyA motif...) that can be use to support the discoverage and reconstruction of this "novel" transcripts that were simulated.
